...
<script>
  ...
  const validate_field = (entry) =>
    field_settings[entry].validate && field_settings[entry].visible;
  ...
  $: valid_routing_number =
    ...
    (routing_number && routing_number.length > 1);
  $: valid_account_number =
    !validate ||
    !validate_field("account_number") ||
    (account_number && account_number.length > 1);
  ...
  <div id="pcivault-ach-form-container" class="ach-form__inner">
    ...
    {#if field_settings.routing_number.visible}
      <div id="pcivault-ach-form-routing-number" class="ach-input">
        {#if routing_number_keypad && !isRetrieval}
          <Keypad
            bind:number={routing_number}
            on:close={() => (routing_number_keypad = false)}
          />
        {/if}
        <label
          id="pcivault-ach-form-routing-number-label"
          for="routing_number"
          class="ach-input__label"
        >
          {$_("form.routing_number.label", { default: "Routing Number" })}
          {#if !valid_routing_number}
            <span
              id="pcivault-ach-form-routing-number-label-error"
              class="ach-input__error"
              >{routing_number
                ? $_("form.routing_number.invalid", {
                    default: "invalid routing number",
                  })
                : $_("form.routing_number.required", {
                    default: "required",
                  })}</span
            >
          {/if}
    ...
    {#if field_settings.account_number.visible}
      <div id="pcivault-ach-form-account-number" class="ach-input">
        {#if account_number_keypad && !isRetrieval}
          <Keypad
            bind:number={account_number}
            on:close={() => (account_number_keypad = false)}
          />
        {/if}
        <label
          id="pcivault-ach-form-account-number-label"
          for="account_number"
          class="ach-input__label"
        >
          {$_("form.account_number.label", { default: "Account Number" })}
          {#if !valid_account_number}
            <span
              id="pcivault-ach-form-account-number-label-error"
              class="ach-input__error"
              >{account_number
                ? $_("form.account_number.invalid", {
                    default: "invalid account number",
                  })
                : $_("form.account_number.required", {
                    default: "required",
                  })}</span
            >
          {/if}
    ...
    {#if field_settings.account_type.visible}
      <div id="pcivault-ach-form-account-type" class="ach-input">
        <label
          id="pcivault-ach-form-account-type-error"
          for="account_type"
          class="ach-input__label"
        >
          {$_("form.account_type.label", { default: "Account Type" })}
          {#if !valid_account_type}
            <span
              id="pcivault-ach-form-account-type-error-label"
              class="ach-input__error"
              >{account_type
                ? $_("form.account_type.invalid", {
                    default: "invalid account type",
                  })
                : $_("form.account_type.required", {
                    default: "required",
                  })}</span
            >
          {/if}
    ...
    {#if field_settings.account_holder.visible}
      <div id="pcivault-ach-form-account-holder" class="ach-input">
        <label
          id="pcivault-ach-form-account-holder-label"
          for="account_holder"
          class="ach-input__label"
        >
          {$_("form.account_holder.label", { default: "Account Holder" })}
          {#if !valid_account_holder}
            <span
              id="pcivault-ach-form-account-holder-label-error"
              class="ach-input__error"
              >{$_("form.account_holder.required", {
                default: "required",
              })}</span
            >
          {/if}
